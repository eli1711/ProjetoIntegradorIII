...
async function testRecoveryRoute() {
    try {
        const response = await fetch('http://10.110.18.11:5000/auth/recuperar_senha_test', {
            method: 'POST',
            headers: {
...
async function sendRecoveryRequest(email, modal, messageElement, recoveryForm) {
    try {
        const response = await fetch('http://10.110.18.11:5000/auth/recuperar_senha', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function handleLogin(event, loginAlert) {
    ...
    try {
        const response = await fetch('http://10.110.18.11:5000/auth/login', {
            method: 'POST',
...
async function testBackendConnection() {
    try {
        const response = await fetch('http://10.110.18.11:5000/auth/test');
        const data = await response.json();
